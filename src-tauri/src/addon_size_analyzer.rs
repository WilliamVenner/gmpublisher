...
use crate::{
	gma::GMAFile,
	transactions::{Transaction, TransactionChannel, Transactions},
	workshop::{self, WorkshopItem},
};
...
impl AddonSizeAnalyzer {
					...
					let treemap = match lock.as_ref() {
						...
						None => {
							...
							let (mut gma_files, gma_ids, total_size) =
								match self.analyze(transaction.channel()) {
									Err(_) => {
										return channel
											.error("ERROR_NO_ADDONS_FOUND", transaction_data!(()))
									}
							...
							gma_files =
								self.download_steam(transaction.channel(), gma_files, gma_ids);
							...
							let treemap =
								self.taggify(gma_files, w, h, total_size, transaction.clone());
			...
			let tag = gma_file
				...
				.or_else(|| {
					if let Some(tags) = &gma_file.gma.metadata.as_ref().unwrap().tags {
						if !tags.is_empty() {
							return Some(tags.get(1).cloned().unwrap());
						}
					}
					None
				})
	...
	pub(super) fn analyze(
		...
		let paths = paths
			.iter()
			.cloned()
			.collect::<Vec<(PathBuf, Option<PublishedFileId>)>>()
			.chunks(chunk_len)
			.map(|c| c.to_owned())
			.collect::<Vec<Vec<(PathBuf, Option<PublishedFileId>)>>>();
			...
			while i < total_paths {
				...
				match rx.recv() {
					Ok(gma) => {
						...
						let pos = match sorted_gmas
							.binary_search_by(|gma_cmp| gma.size.cmp(&gma_cmp.gma.size))
						{
							Ok(pos) => pos,
							Err(pos) => pos,
						};
	...
	pub(crate) fn download_steam(
		&self,
		ws_channel: TransactionChannel,
		gma_files: Vec<AnalyzedAddon>,
		_gma_ids: Vec<Option<PublishedFileId>>,
	) -> Vec<AnalyzedAddon> {
