...
use std::{borrow::Borrow, cell::{Ref, RefCell}, collections::{HashMap, VecDeque}, hash::Hash, marker::PhantomData, sync::{Arc, atomic::{AtomicBool, Ordering}, mpsc::{self, Receiver, Sender}}};
...
pub use self::gma::GMA;
pub use self::steamworks::Steamworks;
	...
	pub fn execute(&self, k: &K, mut v: Args) {
		...
		if let Some(promises) = self.promises(k) {
			match promises {
				Singleton(f) => f(&mut v),
				Variable(vec) => {
					vec.into_par_iter().for_each_with(
						v,
						|v, f| f(v)
					);
				}
...
impl<V: Send + Sync + 'static> RelaxedRwLock<V> {
	pub fn new(v: V) -> RelaxedRwLock<V> {
		let (tx, rx) : (Sender<Box<dyn FnOnce(AtomicRefMutSome<V>) + 'static + Send + Sync>>, Receiver<Box<dyn FnOnce(AtomicRefMutSome<V>) + 'static + Send + Sync>>) = mpsc::channel();
